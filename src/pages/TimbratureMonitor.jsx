                ...
                if (response.ok) {
                    ...
                    const filteredByDate = baseData.filter((item) => {
                        const itemDate = new Date(
                            item.dataOraTimbratura || item.timestamp
                        )
                            .toISOString()
                            .split("T")[0];
                    ...
                    console.log(
                        "Dati filtrati per data lato client:",
                        filteredByDate
                    );
    ...
    const columns = [
        {
            ...
            selector: (row) => {
                ...
                if (
                    row.dipendente &&
                    (row.dipendente.nome || row.dipendente.cognome)
                ) {
                    return `${row.dipendente.nome || ""} ${
                        row.dipendente.cognome || ""
                    }`.trim();
                ...
                // Fallback: mostra ID
                const fallbackId =
                    row.idPersona || row.idTessera || row.id || "N/A";
                return `Dipendente ${fallbackId}`;
            },
            sortable: true,
            grow: 2,
        ...
        },
        {
            ...
            selector: (row) =>
                new Date(
                    row.dataOraTimbratura || row.timestamp
        ...
        },
        {
            name: "Tessera",
            selector: (row) =>
                row.codiceTessera || `ID: ${row.idTessera || "N/A"}`,
            sortable: true,
            grow: 1,
            center: true,
        },
