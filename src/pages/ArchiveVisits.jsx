    ...
    const columns = [
        {
            ...
            cell: (row) => (
                <div
                    title={new Date(row.dataInizio).toLocaleDateString("it-IT")}
                >
                    {new Date(row.dataInizio).toLocaleDateString("it-IT")}
                </div>
            ),
        ...
        },
        {
            ...
            grow: 1,
            cell: (row) => (
                <div title={row.motivoVisita || "N/A"}>
                    {row.motivoVisita || "N/A"}
                </div>
            ),
        },
        {
        ...
        const fetchData = async () => {
            try {
                const response = await apiCall(
                    "http://localhost:8080/api/visite",
                    {
                        method: "GET",
                    }
                );
    ...
    useEffect(() => {
        if (!dateFilter) {
            ...
            const filtered = data.filter((item) => {
                const itemDate = new Date(item.dataInizio).toLocaleDateString(
                    "en-CA"
                ); // YYYY-MM-DD format
